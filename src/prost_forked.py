#!/stor/work/Marcotte/project/ivyfan/operon-graph/env/bin/python3
...
def _search_worker(thr, gothr, qnames, qdb, tnames, tdb_i16, go, goFrq, goDesc, taskInd, n):
    lqdb = len(qdb)
    ldb = len(tdb_i16)
    ...
    for local_i, q in enumerate(qdb[start:stop]):
        qname = parseName(qnames[start + local_i])[0]
        goList[qname] = []
        homologList[qname] = []

        print(f'[{taskInd:02d}] Searching for {qname}')

        # SAFE + FAST: compute L1 distance without allocating (ldb,475) temp per query
        # tdb_i16: (ldb,475) int16
        q_i16 = q.astype(np.int16, copy=False)  # (475,) int16
        dbdiff = np.abs(tdb_i16 - q_i16).sum(axis=1)  # (ldb,)

        m = np.median(dbdiff)
        s = st.median_abs_deviation(dbdiff) * 1.4826
        if s == 0:
            s = 1.0  # avoid div-by-zero
        ...
        dbdiff_res = dbdiff[res] / 2
        evals = e[res]
        names = tnames[res]

        # GO block (unchanged logic)
        if go is not None:
        ...
        for nname, diff, ev in zip(names, dbdiff_res, evals):
            nparsed = parseName(nname)
            homologList[qname].append([nparsed[0], nparsed[1], nparsed[2], nparsed[3], diff, f'{ev:.2e}'])
...
def _search(thr, gothr, querydb, targetdb, godb, n):
    ...
    # Cast ONCE for safe arithmetic (int8 subtraction can overflow)
    # This also speeds things up because we avoid casting per query
    tdb_i16 = tdb.astype(np.int16, copy=False)

    homologList, goList = {}, {}

    with Pool(n) as pool:
        if godb is not None:
            items = [(thr, gothr, qnames, qdb, tnames, tdb_i16, go, goFrq, goDesc, i, n) for i in range(n)]
        else:
            items = [(thr, gothr, qnames, qdb, tnames, tdb_i16, None, None, None, i, n) for i in range(n)]
...
@click.option('-n', '--jobs', default=max(1, os.cpu_count() // 2), help='Number of jobs to run in parallel')
